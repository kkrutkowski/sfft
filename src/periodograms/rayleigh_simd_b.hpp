...
output_data rayleigh_simd_b(const star &data, const FFTGrid &grid, FFT &fft, int &threadID) {
   ...
   for (k=1; k<grid.freq.size(); ++k){
      ...
      */
      ...
      for (i=0; i<n_iter; i++){
         FTA::sincos_ps( _mm256_mul_ps(t_simd[i], _mm256_mul_ps(_mm256_set1_ps(grid.freq[k]), _mm256_set1_ps(twopi))), &C_temp, &S_temp);
