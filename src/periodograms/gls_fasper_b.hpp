...
output_data gls_fasper_b(const star &data, const FFTGrid &grid, FFT &fft) {
   ...
   //std::cout << n << std::endl;
   float* YCYS= fft.NDFT_I_fasper(t, wy, n, grid.fstep, float(1));
   float* C2S2= fft.NDFT_I_fasper(t, w, n, grid.fstep, float(2));
   float* CS = fft.NDFT_I_fasper(t, w, n, grid.fstep, float(1));
   ...
   if (power > best_frequency.power && power < 1.0)
         {
