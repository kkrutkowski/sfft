...
std::tuple<float, float, float> periodogram(FFTGrid &grid, std::filesystem::path in_file, int terms, FFT &fft, int &threadID, uint method){

	star data; data.read(in_file);
	output_data best_frequency;

	if (method == 4){best_frequency = rayleigh_fft_b(data, grid, terms, fft, threadID);} //binned conditional nonuniformity, requires fixing weighting function
