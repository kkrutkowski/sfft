...
/* this header contains two vectorized functions for the data type float (works only for positive floats):
 * 1. avx2::quickselect(float *arr, int n, int k)
 * 2. avx2::quicksort(float *arr, int n)
 * */
...
inline void sort_int_sorting_network(float *arr, int *buff, int n) {
  if(n < 2) return;
  ...
  auto max_pad_vec = _mm256_blendv_epi8(_mm256_set1_epi32(INT32_MAX),_mm256_maskload_epi32(reinterpret_cast<int*>(arr) + idx_max_pad, mask), mask);

  for (int i = 0; i < idx_max_pad / 8; ++i) {
    buffer[i] = LOAD_VECTOR(reinterpret_cast<int*>(arr) + i * 8);
  }
  ...
  for (int i = 0; i < idx_max_pad / 8; i += 1) {
    STORE_VECTOR(reinterpret_cast<int*>(arr) + i * 8, buffer[i]);
  }
  _mm256_maskstore_epi32(reinterpret_cast<int*>(arr) + idx_max_pad, mask, buffer[idx_max_pad / 8]);
}
...
inline int partition_vectorized_8(float *arr, int left, int right,
                                  int pivot, int &smallest, int &biggest) {
  /* make array length divisible by eight, shortening the array */
  for (int i = (right - left) % 8; i > 0; --i) {
    smallest = std::min(smallest, reinterpret_cast<int*>(arr)[left]); biggest = std::max(biggest, reinterpret_cast<int*>(arr)[left]);
...
inline int get_pivot(float *arr, const int left, const int right){
  ...
  for (int i = 0; i < 9; ++i) { /* fill 9 vectors with random numbers */
    ...
    v[i] = _mm256_i32gather_epi32(reinterpret_cast<int*>(arr), result, sizeof(uint32_t)); }
...
inline void qsel_core(float *arr, int left, int right, int k,
                      bool choose_avg = false, const int avg = 0) {
  if (right - left < 256) {
...
inline void quickselect(float *arr, int n, int k) {
